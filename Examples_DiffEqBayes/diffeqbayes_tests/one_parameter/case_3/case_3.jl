...
priors = [
  truncated(Normal(0.6,0.5), 0.1, 3),
  truncated(Normal(1.5,0.5), 0.1, 3),
  truncated(Normal(2.0,0.5), 0.1, 4)
]
...
if success(bayesian_result.return_code)
  ...
  @test mean(p.theta1) ≈ 1.0 atol=5e-1
  @test mean(p.theta2) ≈ 1.0 atol=5e-1
  @test mean(p.theta3) ≈ 1.5 atol=5e-1
  ...
  plot(p1[1], p2[1], p1[2], p2[2], p1[3], p2[3], layout=(length(priors),2))
  savefig("$(@__DIR__)/case_3_chains.png")
  ...
  savefig("$(@__DIR__)/case_3_pred_prey.png")
end
