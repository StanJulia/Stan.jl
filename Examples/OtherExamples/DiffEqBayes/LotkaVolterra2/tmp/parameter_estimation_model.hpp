...
stan::io::program_reader prog_reader__() {
    ...
    reader.add_event(35, 35, "end", "/Users/rob/.julia/v0.6/Stan/Examples/OtherExamples/DiffEqBayes/LotkaVolterra2/tmp/parameter_estimation_model.stan");
    return reader;
}
...
public:
    ...
    void ctor_body(stan::io::var_context& context__,
        ...
        try {
            ...
            validate_non_negative_index("params", "2", 2);
            num_params_r__ += 2;
        } catch (const std::exception& e) {
            stan::lang::rethrow_located(e, current_statement_begin__, prog_reader__());
            // Next line prevents compiler griping about no return
    ...
    void transform_inits(const stan::io::var_context& context__,
        ...
        pos__ = 0U;
        validate_non_negative_index("theta", "4", 4);
        context__.validate_dims("initialization", "theta", "double", context__.to_vec(4));
        std::vector<double> theta(4,double(0));
        ...
        if (!(context__.contains_r("params")))
            throw std::runtime_error("variable params missing");
        vals_r__ = context__.vals_r("params");
        pos__ = 0U;
        validate_non_negative_index("params", "2", 2);
        context__.validate_dims("initialization", "params", "double", context__.to_vec(2));
        std::vector<double> params(2,double(0));
        for (int i0__ = 0U; i0__ < 2; ++i0__)
            params[i0__] = vals_r__[pos__++];
        for (int i0__ = 0U; i0__ < 2; ++i0__)
            try {
            writer__.scalar_unconstrain(params[i0__]);
        } catch (const std::exception& e) { 
            throw std::runtime_error(std::string("Error transforming variable params: ") + e.what());
        }
    ...
    T__ log_prob(vector<T__>& params_r__,
        ...
        try {
            ...
            vector<local_scalar_t__> params;
            size_t dim_params_0__ = 2;
            params.reserve(dim_params_0__);
            for (size_t k_0__ = 0; k_0__ < dim_params_0__; ++k_0__) {
                if (jacobian__)
                    params.push_back(in__.scalar_constrain(lp__));
                else
                    params.push_back(in__.scalar_constrain());
            }
            ...
            lp_accum__.add(inv_gamma_log<propto__>(get_base1(params,2,"params",1), 2.0, 3.0));
            current_statement_begin__ = 30;
            lp_accum__.add(normal_log<propto__>(get_base1(theta,1,"theta",1), 1.5, 1.0));
            ...
            else lp_accum__.add(-log_diff_exp(normal_cdf_log(2.0, 1.5, 1.0), normal_cdf_log(0.0, 1.5, 1.0)));
            current_statement_begin__ = 30;
            lp_accum__.add(normal_log<propto__>(get_base1(theta,2,"theta",1), 1.0, 1.0));
            if (get_base1(theta,2,"theta",1) < 0.0) lp_accum__.add(-std::numeric_limits<double>::infinity());
            else if (get_base1(theta,2,"theta",1) > 2.0) lp_accum__.add(-std::numeric_limits<double>::infinity());
            else lp_accum__.add(-log_diff_exp(normal_cdf_log(2.0, 1.0, 1.0), normal_cdf_log(0.0, 1.0, 1.0)));
            current_statement_begin__ = 30;
            lp_accum__.add(normal_log<propto__>(get_base1(theta,3,"theta",1), 3.0, 1.0));
            current_statement_begin__ = 30;
            lp_accum__.add(normal_log<propto__>(get_base1(theta,4,"theta",1), 1.0, 1.0));
            ...
            for (int t = 1; t <= T; ++t) {
                ...
                lp_accum__.add(normal_log<propto__>(get_base1(internal_var___u,t,"internal_var___u",1), get_base1(u_hat,t,"u_hat",1), get_base1(params,2,"params",1)));
            }
            }
